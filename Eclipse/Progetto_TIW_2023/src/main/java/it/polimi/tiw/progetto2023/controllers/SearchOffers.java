...
public class SearchOffers extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(req.getParameter("productId") != null) {
			...
			/*
			 * Check if the selected product is in the list of the products on screen.
			 * The ID may not be in the list if the user modified it from the URL bar.
			 * Without this check the Servlet will not load the "impostor" product, but
			 * will add it to the visualized products list. 
			 */
			...
			/*
			 * If the product wasn't in the list, the Servlet will redirect to SearchProduct
			 * which is more appropriate in this situation.
			 */
			if(!present) {
				resp.sendRedirect(getServletContext().getContextPath() + "/SearchProduct?productId=" + Integer.parseInt(req.getParameter("productId")));
				return;
			}
			
			/*
			 * Adding the clicked product to the previously viewed products.
			 */
